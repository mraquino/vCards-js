		...
		getFormattedString: function(vCard) {
			...
			if (vCard.workCellPhone) {
				...
				vCard.workPhone.forEach(
					function(number) {
						if (majorVersion >= 4) {
							formattedVCardString += 'TEL;VALUE=uri;TYPE="voice,work,cell":tel:' + e(number) + nl();
			...
			if (vCard.workUrl) {
				formattedVCardString += 'URL;type=' + vCard.organization?vCard.organization:'WORK' + encodingPrefix + ':' + e(vCard.workUrl) + nl();
			}
